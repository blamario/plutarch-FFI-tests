...
watch: requires_nix_shell
	while sleep 1; do find src plutarch-ffi-tests.cabal | entr -cd make build; done
...
ghci: requires_nix_shell
	cabal v2-repl $(GHC_FLAGS) plutarch-ffi-tests
...
build_path = dist-newstyle/build/x86_64-linux/ghc-8.10.4.20210212/plutarch-ffi-tests-0.1
clear_build:
